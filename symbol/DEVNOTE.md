...
4. `T * Note::getXXX(size_t pos) const` methods throw `out_of_range` exception in case of range errors, but does not check `nullptr`.
5. The user must check beforehand, either with `getXXX(pos) == nullptr` or using `hasXXX(pos)`.
6. `void setTied(pos)` and `void isTied(pos)` method check the range and throw `out_of_range` exception
