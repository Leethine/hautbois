...
class Acciaccatura : public GraceNote {
 ...
 public:
  ...
  Acciaccatura(int __num, int __denom, const char * __pitch,
               const std::initializer_list<const char *> __args);

  Acciaccatura(int __num, int __denom, const std::string& __pitch,
               const std::vector<std::string>& __args);

  Acciaccatura(const Acciaccatura& __other);

  Acciaccatura(const Acciaccatura&& __other);

  virtual Acciaccatura& operator=(const Acciaccatura& __other);
