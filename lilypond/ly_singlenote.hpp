...
#include "../symbol/note_single.hpp"

#include <string>

#ifndef LY_SINGLENOTE_H
#define LY_SINGLENOTE_H

namespace hautbois {
namespace ly {

class LySingleNote : public core::SingleNote {
 ...
 public:
  ...
  LySingleNote(const std::string& __pitch, const std::string& __duration);

  LySingleNote(const std::string& __duration);

  LySingleNote(const LySingleNote& __note);
  
  LySingleNote(const LySingleNote&& __note);

  LySingleNote(const core::SingleNote& __note);
  
  LySingleNote(const core::SingleNote&& __note);

  virtual LySingleNote& operator=(const LySingleNote& __n)=delete;
  virtual LySingleNote& operator=(const core::SingleNote& __n)=delete;

  virtual ~LySingleNote();
